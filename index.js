async function embedImpactBubble() {

  if (window.impactConf?.embedSuccess) {
    return;
  }
  ...
  chatButton.setAttribute('id', 'impact-bubble-button')
  ...
  messageBubbles.setAttribute('id', 'impact-message-bubbles')
  ...
  chat.setAttribute('id', 'impact-bubble-window')
  ...
  const getChatbotStyles = async () => {
    const response = await fetch(
      `https://getimpact.ai/api/chatbot/styles?id=${scriptTag.id}`,
      {
        method: 'GET',
        headers: {
  ...
  if (!window.impactConf) {
    window.impactConf = { embedSuccess: false };
  }

  if (window.impactConf) {
    window.impactConf.embedSuccess = true;
  }
}
